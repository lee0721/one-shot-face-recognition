...
print("Setting up the **embedding generator model")
...
def euclidean_dist(inputs):
    ...
    u, v = inputs
    return tf.sqrt(tf.reduce_sum(tf.square(u - v), axis=1, keepdims=True))
...
x = Lambda(lambda x: tf.nn.l2_normalize(x, axis=1), name='l2_normalize', output_shape=lambda s: s)(x)
x = Dense(64)(x)
...
print("Inspecting what the network has learned")
...
for model_index in range (1, model_num+1) :
  ...
  if model_index < trip_model_num+1 :  # 要看前幾個是tri
    ...
    # --------------------- 先得到所有Train的embeddings ---------------------
    train_embeddings = {}

    for subfolder in os.listdir(train_folder):

      class_image = join(train_folder, subfolder)
      dir_image = os.listdir( class_image )
      dir_image.sort()
      full_img_path = join(class_image, dir_image[2])

      # predict
      embedding = tri_face_recognition(full_img_path)
      train_embeddings[subfolder] = embedding

    filename = "embedding" + str(model_index) + ".pkl"
    with open(filename, "wb") as f:
      pickle.dump(train_embeddings, f)

    print( "Trian的embeddings計算完\n" )
  ...
  # ==================================================== Contrastive Network ====================================================
    ...
    for i in range(0,people_num,2):
      class_image1 = join(train_folder, str(i))
      dir_image1 = os.listdir( class_image1 )
      dir_image1.sort()
      path_image1 = join(class_image1, dir_image1[2])

      if people_num % 2 != 0 and i == people_num-1:
        j = i
      else:
        j = i + 1
      class_image2 = join(train_folder, str(j))
      dir_image2 = os.listdir( class_image2 )
      dir_image2.sort()
      path_image2 = join(class_image2, dir_image2[2])

      if people_num % 2 != 0 and i == people_num-1:
        train_embeddings[i], trash = ssl_face_recognition(path_image1, path_image2)
      else:
        train_embeddings[i], train_embeddings[i+1] = ssl_face_recognition(path_image1, path_image2)
